...
Coverage is currently calculated for `js/**/*.js`, excluding the simple entrypoint shims in `js/entries/*.js`.
...
|---|---:|---|
| `js/timer-state.js` | 96.53% | Core timer state transitions are pure and heavily unit-tested. |
| `js/settings-validation.js` | 94.53% | Validation rules are deterministic and covered with positive and negative cases. |
| `js/productivity-store.js` | 93.15% | Storage normalization and CRUD logic are covered through unit tests. |
| `js/app-status.js` | 100% | The environment-status DOM write is directly unit-tested. |
| `js/worker.js` | 100% | Worker start/stop/tick behavior is unit-tested with mocked timers. |

## Low-Coverage Modules

| File | Statements | Reason |
|---|---:|---|
| `js/timer.js` | 0% | Large browser coordinator module; behavior is currently validated through Playwright rather than unit tests. |
| `js/settings.js` | 0% | DOM-driven settings controller with many event listeners; covered indirectly by browser tests. |
| `js/productivity.js` | 0% | Page-level UI controller; CRUD and validation flows are exercised in e2e tests instead of isolated unit tests. |
| `js/mini-timer.js` | 0% | Cross-page header widget is validated through browser sync tests, not unit tests. |
| `js/sound.js` | 0% | Depends on browser audio APIs and autoplay policies; best validated with browser-level behavior tests. |
...
- `js/settings.js`
  The validation rules themselves are already covered in `js/settings-validation.js`, but the controller that binds those rules to DOM fields is not. Adding targeted unit tests for error rendering, field normalization, and event dispatch would be useful.
